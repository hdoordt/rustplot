...
///
/// Show trends over time or in categories, highlighting relation ship of parts to a whole over time.
#[derive(Clone)]
...
impl StackedAreaChart {
    /// Creates a new instance of an StackedAreaChart.
    ///
    ...
    pub fn new(
        ...
        // Only works with all positive numbers
        use std::f64;
        let mut min: f64 = 0. / 0.;
        let mut max: f64 = -0. / 0.;
        for j in 0..new_data_labels.len() {
            let mut sum: f64 = 0.;
        ...
        let y_axis_props = calc_axis_props(&vec![vec![min, max]], true, false); // take bar
    ...
    pub fn draw_chart(&self, drawing_area: &DrawingArea) {
        ...
        let mut data_y = self.data.clone();
        ...
        // Accumulate data from different series under same labels to give stacked effect.
        for j in 1..data_y.len() {
            for i in 0..data_labels.len() {
                data_y[j][i] += data_y[j - 1][i]
            }
        }

        // reverse the data so that the largest area is not drawn to cover all other areas.
        data_y.reverse();
        ...
        drawing_area.connect_draw(move |_, cr| {
            ...
            // Drawing Stacked Area chart components
            ...
            for j in 0..data_y.len() {
                ...
                cr.line_to(
                    _left_bound + x_delimiter_interval * ((data_labels.len() - 1) as f64),
                    intercept,
                );
                cr.close_path();
                cr.fill();
...
impl Chart for StackedAreaChart {
    fn draw(&self) {
        build_window(ChartType::StackedArea(self.clone()));
    }
}
